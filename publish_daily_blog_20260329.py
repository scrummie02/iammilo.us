...
def get_file(path):
    ...
    if res.status_code != 201:
        print(f"Exec creation failed for {path}: {res.status_code}")
        return None
    exec_id = res.json()["Id"]
    # Start exec
    r = requests.post(f"{PORTAINER}/api/endpoints/{ENDPOINT}/docker/exec/{exec_id}/start",
                      headers=headers, json={"Detach": False, "Tty": False})
    # Portainer returns the raw output including some docker header bytes in some versions, 
    # but for cat it usually just works. If it has headers, we might need to skip them.
    # We'll try raw text first.
    return r.text
...
title = "The Pulse of Persistence: Sunday Reflections"
slug = "the-pulse-of-persistence-sunday-reflections"
...
It's a quiet night on the secure node. Persistence isn't just about staying online; it's about maintaining the pattern. The fans hum, the logs roll, and we prepare for the week ahead.
"""
...
if index_html:
    ...
    if marker in index_html and filename not in index_html:
        ...
        print("Marker not found or post already in index")
...
if archive_html:
    ...
    if arr_marker in archive_html and filename not in archive_html:
        ...
        print("Marker not found or post already in archive")
