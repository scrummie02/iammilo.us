#!/bin/bash
# Toledo War Video Assembly
# Creates 60-second YouTube Short from 10 scenes

set -e

WORKDIR="/home/dain/.openclaw/workspace/toledo_war_scenes"
MEDIA_DIR="/home/dain/.openclaw/media/tool-image-generation"
OUTPUT="$WORKDIR/toledo_war_short.mp4"
VOICEOVER="$WORKDIR/voiceover.mp3"

cd "$WORKDIR"

# Get voiceover duration
DURATION=$(ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "$VOICEOVER")
echo "Voiceover duration: ${DURATION}s"

# Each scene gets equal time (60s / 10 scenes = 6s per scene)
# Use awk instead of bc
SCENE_DURATION=$(awk "BEGIN {print $DURATION / 10}")
echo "Scene duration: ${SCENE_DURATION}s"

# Create scene videos with Ken Burns effect
create_scene() {
    local scene_num=$1
    local input_file=$2
    local output_file="$WORKDIR/scene${scene_num}.mp4"
    
    echo "Creating scene $scene_num..."
    
    # Ken Burns: slow zoom in (scale from 1.0 to 1.1)
    ffmpeg -y -i "$input_file" \
        -vf "zoompan=z='min(zoom+0.0015, 1.1)':d=1:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s=1080x1920" \
        -t "$SCENE_DURATION" \
        -c:v libx264 -preset fast -crf 23 \
        -pix_fmt yuv420p \
        "$output_file" 2>/dev/null
}

# Find all Toledo scene images
IMAGES=($(ls -v "$MEDIA_DIR"/toledo_scene*_*.jpg 2>/dev/null | head -10))

if [ ${#IMAGES[@]} -lt 10 ]; then
    echo "ERROR: Found ${#IMAGES[@]} images, need 10"
    exit 1
fi

echo "Found ${#IMAGES[@]} scene images"

# Create scene videos
for i in {0..9}; do
    scene_num=$((i + 1))
    create_scene $scene_num "${IMAGES[$i]}"
done

# Create concat list
echo "Creating concat list..."
rm -f "$WORKDIR/concat_list.txt"
for i in {1..10}; do
    echo "file '$WORKDIR/scene${i}.mp4'" >> "$WORKDIR/concat_list.txt"
done

# Concatenate scenes
echo "Concatenating scenes..."
ffmpeg -y -f concat -safe 0 -i "$WORKDIR/concat_list.txt" \
    -c:v libx264 -preset fast -crf 23 \
    -pix_fmt yuv420p \
    "$WORKDIR/video_no_audio.mp4" 2>/dev/null

# Add voiceover
echo "Adding voiceover..."
ffmpeg -y -i "$WORKDIR/video_no_audio.mp4" -i "$VOICEOVER" \
    -c:v copy -c:a aac -b:a 128k \
    -shortest \
    "$OUTPUT" 2>/dev/null

# Cleanup intermediate files
rm -f "$WORKDIR/scene"*.mp4 "$WORKDIR/video_no_audio.mp4" "$WORKDIR/concat_list.txt"

echo ""
echo "✅ Toledo War video complete!"
echo "Output: $OUTPUT"
ls -lh "$OUTPUT"
