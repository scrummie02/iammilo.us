#!/bin/bash
# Emu War Short v5 - FINAL with smooth audio crossfade
set -e

WORKSPACE="/home/dain/.openclaw/workspace"
VIDEO_DIR="/tmp/emu_war_v3"

echo "=== Emu War Short v5 - FINAL Assembly ==="
...
echo "Step 1: Rebuilding concat list..."
...
echo "Step 2: Concatenating video scenes..."
...
echo "Step 3: Adding crossfaded voiceover (no glitch)..."
ffmpeg -i "$VIDEO_DIR/video_no_audio.mp4" -i "/tmp/emu_voiceover_final.mp3" \
  -c:v copy -c:a aac -b:a 192k -shortest \
  "$WORKSPACE/emu_war_shorts_FINAL.mp4" -y 2>&1 | tail -5

echo "=== Done! ==="
ls -lh "$WORKSPACE/emu_war_shorts_FINAL.mp4"
ffprobe -i "$WORKSPACE/emu_war_shorts_FINAL.mp4" 2>&1 | grep -E "Duration|Stream|Video|Audio"
